...
export class Combobox extends SpectrumElement {
    ...
    @property()
    public autocomplete: 'list' | 'none' = 'list';
    ...
    public onListClick(event: PointerEvent & { target: ComboboxItem }): void {
        const { target } = event;
        event.preventDefault();
        this.activeDescendent = target;
        if (!this.activeDescendent) {
    ...
    private positionListboxFromEntries(entries: MutationRecord[]): void {
        entries.forEach((entry) => {
            if ((entry.target as HTMLElement).children.length) {
                this.positionListbox(((entry.target as HTMLElement).children[0] as HTMLElement));
            }
        });
    }

    private positionListbox(listbox: HTMLElement): void {
        const targetRect = listbox.getBoundingClientRect();
    ...
    protected render(): TemplateResult {
        ...
        return html`
            <sp-field-label id="label" for="input" side-aligned="start">
            ...
            <sp-action-button
                ...
                @click=${this.toggleOpen}
                tabindex="-1"
                class="button ${this.focused ? 'focus-visible' : ''}"
